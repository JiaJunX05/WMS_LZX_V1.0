...
class GenderController extends Controller
{
     ...
     * 導出性別數據到Excel
     */
    public function exportGenders(Request $request)
    {
        try {
            // 獲取篩選條件
            $filters = [
            ...
            $filename = "genders_export_{$timestamp}.xlsx";

            // 使用Laravel Excel導出
            return Excel::download(new GenderExport($filters), $filename);

        } catch (\Exception $e) {
            Log::error('Gender export failed: ' . $e->getMessage());
