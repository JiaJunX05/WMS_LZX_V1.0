...
class TemplateController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('admin.sizes.templates.dashboard', compact('sizeTemplates', 'categories', 'genders'));
    }
    ...
    public function create()
    {
        ...
        return view('admin.sizes.templates.create', compact('categories', 'genders', 'sizeLibraries'));
    }
    ...
    public function edit($id)
    {
        try {
            ...
            return view('admin.sizes.templates.update', compact('sizeTemplate', 'categories', 'genders', 'sizeLibraries'));
        } catch (\Exception $e) {
    ...
    public function update(Request $request, $id)
    {
        try {
            ...
            if ($existingTemplate) {
                return $this->handleDuplicateTemplate($request, 'This template combination already exists');
            }
    ...
    public function getAvailableSizeLibraries(Request $request)
    {
        try {
            ...
            if ($categoryId == 0 || $genderId == 0) {
                ...
                // 如果该类别下没有尺码库，则获取所有可用的尺码库
                if ($sizeLibraries->isEmpty()) {
                    $sizeLibraries = SizeLibrary::where('size_status', 'Available')
                        ->with('category')
                        ->orderBy('category_id')
                        ->orderBy('id')
        ...
        try {
            ...
            return response()->json([
                ...
                'pagination' => [
                    'current_page' => 1,
                    'last_page' => 1,
                    'per_page' => $groupedTemplates->count(),
                    'total' => $groupedTemplates->count(),
                    'from' => 1,
                    'to' => $groupedTemplates->count(),
                ]
            ]);
        } catch (\Exception $e) {
    ...
    private function viewTemplateGroup($id)
    {
        ...
        if (count($parts) === 2) {
            ...
            if ($category && $gender) {
                ...
                return view('admin.sizes.templates.view', compact('sizeTemplates', 'categories', 'genders', 'sizeLibraries', 'category', 'gender'));
            }
        }
    ...
    private function viewSingleTemplate($id)
    {
        ...
        return view('admin.sizes.templates.view', compact('sizeTemplate', 'categories', 'genders', 'sizeLibraries'));
    }
