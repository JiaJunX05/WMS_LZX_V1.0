...
class GenderManagement {
    ...
    updateStatistics(response) {
        ...
        if (response.data) {
            ...
            const withSizesCount = response.data.filter(gender => gender.sizes_count > 0).length;
    ...
    createGenderRow(gender) {
        const statusMenuItem = gender.gender_status === 'Unavailable'
            ? `<a class="dropdown-item" href="javascript:void(0)" onclick="genderManagement.setAvailable(${gender.id})">
                   <i class="bi bi-check-circle me-2"></i> Activate Gender
               </a>`
            : `<a class="dropdown-item" href="javascript:void(0)" onclick="genderManagement.setUnavailable(${gender.id})">
                   <i class="bi bi-slash-circle me-2"></i> Deactivate Gender
               </a>`;

        const actionButtons = `
            <button class="btn-action" title="Edit" onclick="genderManagement.editGender(${gender.id})">
            ...
            <div class="btn-group dropend d-inline">
                ...
                <ul class="dropdown-menu">
                    ...
                    <li>
                        <a class="dropdown-item text-danger" href="javascript:void(0)" onclick="genderManagement.deleteGender(${gender.id})">
        ...
        // 获取性别显示样式
        const genderClass = this.getGenderClass(gender.gender_name);
            ...
            <tr>
                ...
                <td>
                    <div class="d-flex align-items-center">
                        <span class="gender-name-badge ${genderClass}">${gender.gender_name}</span>
                    </div>
                </td>
                <td>
                    <span class="sizes-count-badge">${gender.sizes_count}</span>
                </td>
    ...
    getGenderClass(genderName) {
        ...
        return genderMap[genderName] || 'default';
    }
    ...
    deleteGender(genderId) {
        this.submitForm(
            window.deleteGenderUrl.replace(':id', genderId),
            'DELETE',
    ...
    setAvailable(genderId) {
        this.submitForm(
            window.availableGenderUrl.replace(':id', genderId),
            'PATCH',
            'Are you sure you want to activate this gender?'
        );
    }
    ...
    setUnavailable(genderId) {
        this.submitForm(
            window.unavailableGenderUrl.replace(':id', genderId),
            'PATCH',
            'Are you sure you want to deactivate this gender?'
        );
    }
...
// 初始化所有Gender管理相关功能
function initializeGenderManagement() {
    // 设置状态卡片选择功能
...
// =============================================================================
let genderManagement;
$(document).ready(function() {
    // 检查当前页面类型并初始化相应功能

    // 如果是dashboard页面（有table-body元素）
    if ($("#table-body").length > 0) {
        genderManagement = new GenderManagement();
    }

    // 初始化Gender Management
    initializeGenderManagement();
});
