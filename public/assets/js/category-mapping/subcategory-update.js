...
function setupFormSubmission() {
    // 处理 Subcategory Update 表单提交
    const updateForm = document.querySelector('form[action*="update"]');
    if (updateForm) {
        console.log('Subcategory update form found:', updateForm);
        updateForm.addEventListener('submit', handleSubcategoryUpdateSubmit);
    } else {
        console.log('Subcategory update form not found');
    }
}

function handleSubcategoryUpdateSubmit(e) {
    e.preventDefault();
    ...
    .then(data => {
        if (data.success) {
            ...
            setTimeout(() => {
                window.location.href = window.subcategoryManagementRoute || '/admin/subcategories';
            }, 1500);
        } else {
    ...
    .catch(error => {
        ...
        if (error.message.includes('already exists')) {
            showAlert('This subcategory name already exists. Please choose a different name.', 'warning');
        } else {
