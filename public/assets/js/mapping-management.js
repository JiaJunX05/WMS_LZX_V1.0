...
function renderCategoryCards(mappings) {
    ...
    if (!mappings || mappings.length === 0) {
        ...
        allCategoryGroups = [];
        updatePaginationInfoByCategory(mappings, null);
        return;
    }
    ...
    allCategoryGroups = Object.keys(groupedByCategory).map(categoryId => {
        ...
        return {
            ...
            mappings: categoryData.mappings
        };
    });
...
function displaySubcategoryCards(subcategories) {
    ...
    container.innerHTML = subcategories.map(subcategory => {
        const imageUrl = subcategory.subcategory_image
            ? `/assets/images/${subcategory.subcategory_image}`
            : null;
        const imageHtml = imageUrl
            ? `<img src="${imageUrl}"
                     alt="${subcategory.subcategory_name}"
                     class="subcategory-card-image mb-2"
                     style="width: 100%; height: 120px; object-fit: cover; border-radius: 8px;"
                     onerror="this.onerror=null; this.style.display='none'; this.nextElementSibling.style.display='flex';">
                <div class="subcategory-card-placeholder d-none" style="width: 100%; height: 120px; background: linear-gradient(135deg, #f5f7fa 0%, #c3cfe2 100%); border-radius: 8px; display: flex; align-items: center; justify-content: center; margin-bottom: 0.5rem;">
                    <i class="bi bi-image text-muted" style="font-size: 2rem;"></i>
                </div>`
            : `<div class="subcategory-card-placeholder mb-2" style="width: 100%; height: 120px; background: linear-gradient(135deg, #f5f7fa 0%, #c3cfe2 100%); border-radius: 8px; display: flex; align-items: center; justify-content: center;">
                <i class="bi bi-image text-muted" style="font-size: 2rem;"></i>
            </div>`;
        ...
        <div class="col-md-3 col-sm-4 col-6 mb-3">
            <div class="card subcategory-card h-100 border border-light position-relative"
                ...
                <div class="card-body d-flex flex-column justify-content-center align-items-center text-center p-3"
                    ...
                    ${imageHtml}
                    <div class="size-value fw-bold text-dark mb-0" style="font-size: 0.9rem; line-height: 1.2;">${subcategory.subcategory_name.toUpperCase()}</div>
                </div>
            </div>
        </div>
    `;
