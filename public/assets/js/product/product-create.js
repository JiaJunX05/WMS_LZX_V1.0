// =============================================================================
// 产品创建页面 JavaScript (Product Create Page JavaScript)
// 功能：图片预览、级联选择、容量检查、SKU生成、状态选择
// =============================================================================
...
// =============================================================================
// Rack容量管理功能 (Rack Capacity Management)
// =============================================================================

// 绑定货架容量检查功能
function bindRackCapacityCheck() {
    const rackSelect = document.getElementById('rack_id');     // 货架选择框
    const quantityInput = document.getElementById('quantity'); // 数量输入框

    if (rackSelect && quantityInput) {
        // 监听货架选择变化事件
        rackSelect.addEventListener('change', function() {
            const selectedOption = this.options[this.selectedIndex];
            if (selectedOption && selectedOption.dataset.available) {
                const available = parseInt(selectedOption.dataset.available); // 可用容量
                const capacity = parseInt(selectedOption.dataset.capacity);   // 总容量
                const used = parseInt(selectedOption.dataset.used);           // 已使用容量

                // 设置数量输入框的最大值
                quantityInput.setAttribute('max', available);
                showRackCapacityInfo(available, capacity, used);
            }
        });
        ...
        quantityInput.addEventListener('input', function() {
            ...
            if (selectedOption && selectedOption.dataset.available) {
                ...
                const requested = 1; // 每个产品占用1个位置

                if (requested > available) {
                    this.setCustomValidity(`Only ${available} product slots available in this rack`);
                    showRackCapacityWarning(available, requested);
                } else {
                    this.setCustomValidity('');
                    hideRackCapacityWarning();
                }
            }
        });
    }
...
// 显示货架容量信息
function showRackCapacityInfo(available, capacity, used) {
    let capacityInfo = document.getElementById('rack-capacity-info');
    if (!capacityInfo) {
        capacityInfo = document.createElement('div');
        capacityInfo.id = 'rack-capacity-info';
        capacityInfo.className = 'alert alert-info mt-2';
        const rackSelect = document.getElementById('rack_id');
        if (rackSelect && rackSelect.parentNode) {
            rackSelect.parentNode.appendChild(capacityInfo);
        }
    }

    capacityInfo.innerHTML = `
        <i class="bi bi-info-circle me-2"></i>
        <strong>Rack Capacity:</strong> ${used}/${capacity} products used, ${available} product slots available
        <div class="progress mt-2" style="height: 6px;">
            <div class="progress-bar" role="progressbar" style="width: ${(used/capacity)*100}%"></div>
        </div>
    `;
}

// 显示货架容量警告
function showRackCapacityWarning(available, requested) {
    let warning = document.getElementById('rack-capacity-warning');
    if (!warning) {
        warning = document.createElement('div');
        warning.id = 'rack-capacity-warning';
    ...
    warning.innerHTML = `
        ...
        <strong>Capacity Exceeded:</strong> Requested ${requested} product slot, but only ${available} product slots available in this rack.
    `;
}

// 隐藏货架容量警告
function hideRackCapacityWarning() {
    const warning = document.getElementById('rack-capacity-warning');
    if (warning) {
        warning.remove();
    }
}
