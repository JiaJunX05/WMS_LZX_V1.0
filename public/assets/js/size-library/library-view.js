...
function bindEvents() {
    // 刪除按鈕事件
    const deleteButtons = document.querySelectorAll('[onclick*="deleteLibrary"]');
    deleteButtons.forEach(button => {
        button.addEventListener('click', function(e) {
            e.preventDefault();
            const sizeId = this.getAttribute('onclick').match(/\d+/)[0];
            const sizeValue = this.getAttribute('onclick').match(/'([^']+)'/)[1];
            deleteLibrary(sizeId, sizeValue);
        });
    });
}
...
function deleteLibrary(sizeId, sizeValue) {
    ...
    deleteLibrary(sizeId,
        function(data) {
            ...
            // 刪除成功後，從頁面中移除該行
            const deletedRow = document.querySelector(`[onclick*="deleteLibrary(${sizeId}"]`).closest('tr');
            if (deletedRow) {
                deletedRow.remove();
            }
