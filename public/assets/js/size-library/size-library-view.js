/**
 * Size Library View Page JavaScript
 * 尺码库查看页面交互逻辑
 */
...
function deleteSize(sizeId, sizeValue) {
    ...
    .then(data => {
        if (data.success) {
            ...
            // 删除成功后，从页面中移除该行
            const deletedRow = document.querySelector(`[onclick*="deleteSize(${sizeId}"]`).closest('tr');
            if (deletedRow) {
                deletedRow.remove();
            }
