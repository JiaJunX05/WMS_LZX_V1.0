/**
 * Size Template View Page JavaScript
 * 尺码模板查看页面交互逻辑
 */
...
function deleteTemplate(templateId, sizeValue) {
    ...
    // 发送删除请求
    const deleteUrl = window.deleteTemplateUrl.replace(':id', templateId);
    ...
    .catch(error => {
        ...
        if (error.message.includes('200') || error.message.includes('302')) {
            ...
            showAlert('An error occurred while deleting the template: ' + error.message, 'error');
        }
    })
    .finally(() => {
        // 重置删除状态
