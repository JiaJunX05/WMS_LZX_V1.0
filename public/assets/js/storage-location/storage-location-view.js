/**
 * Storage Location View Page JavaScript
 * 存储位置查看页面交互逻辑
 */
...
function deleteLocation(locationId, zoneName, rackNumber) {
    ...
    // 发送删除请求
    const deleteUrl = window.deleteLocationUrl.replace(':id', locationId);
    ...
    .then(data => {
        if (data.success) {
            ...
            showAlert(data.message || 'Failed to delete location', 'error');
        }
    })
    .catch(error => {
        ...
        if (error.message.includes('200') || error.message.includes('302')) {
            ...
            showAlert('An error occurred while deleting the location: ' + error.message, 'error');
        }
    })
    .finally(() => {
        // 重置删除状态
