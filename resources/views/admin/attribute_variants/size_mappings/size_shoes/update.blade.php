...
@section("title", "Update Shoe Size")
@section("content")
                ...
                <div class="row align-items-center">
                    ...
                    <div class="col-lg-8">
                        <div class="d-flex align-items-center">
                            ...
                            <div>
                                <h2 class="dashboard-title mb-1">Update Shoe Size</h2>
                                <p class="dashboard-subtitle mb-0">Modify existing shoe size information</p>
                            </div>
                        </div>
                    </div>
                    {{-- 操作按钮区域 --}}
        ...
        <div class="row g-0">
                ...
                <div class="preview-section d-flex flex-column h-100 bg-light p-3">
                    <div class="d-flex justify-content-between align-items-center mb-3">
                        <h6 class="mb-0 fw-bold text-primary">
                            <i class="bi bi-tags me-2"></i>Shoe Size Information
                        </h6>
                    </div>
                    <div class="preview-container flex-grow-1 d-flex align-items-center justify-content-center">
                        <div class="text-center">
                            ...
                            <p class="text-muted mt-3">Shoe Size Management</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="col-md-7">
                <div class="card-body p-4">
                    <!-- 表单标题 -->
                    <h2 class="text-primary text-center mb-3">Update Shoe Size</h2>
                    <p class="text-muted text-center">Modify existing shoe size information</p>
                    <hr>

            <!-- 表单内容 -->
            <form action="{{ route('admin.attribute_variant.size_shoes.update', $sizeShoes->id) }}" method="post">
                @csrf
                @method('PUT')
                        ...
                        <div class="mb-4">
                            ...
                            <div class="input-group">
                                ...
                                <input type="text" class="form-control border-start-0" id="size_value" name="size_value"
                                       value="{{ $sizeShoes->size_value }}" required>
                            </div>
                        </div>
                    </div>
                            ...
                            <div class="input-group">
                                ...
                                <select class="form-control border-start-0" id="gender_id" name="gender_id" required>
                                    ...
                                    @foreach($genders as $gender)
                                        <option value="{{ $gender->id }}" {{ $sizeShoes->gender_id == $gender->id ? 'selected' : '' }}>{{ $gender->gender_name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                ...
                <div class="mb-4">
                    ...
                    <div class="input-group">
                        ...
                        <textarea class="form-control border-start-0" id="measurements" name="measurements"
                                  placeholder="Enter detailed measurements (e.g., length: 25cm, width: 9cm)" rows="3">{{ is_array($sizeShoes->measurements) && isset($sizeShoes->measurements['description']) ? $sizeShoes->measurements['description'] : '' }}</textarea>
                    </div>
                </div>
                ...
                <div class="mb-4">
                    ...
                    <div class="row g-3">
                        <div class="col-md-6">
                            <div class="card h-100 border status-card {{ $sizeShoes->size_status === 'Available' ? 'selected' : '' }}" data-status="Available">
                                <label class="card-body d-flex align-items-center" style="cursor: pointer;">
                                    <input type="radio" name="size_status" value="Available" class="form-check-input me-3" {{ $sizeShoes->size_status === 'Available' ? 'checked' : '' }}>
                                    <div>
                        ...
                        <div class="col-md-6">
                            <div class="card h-100 border status-card {{ $sizeShoes->size_status === 'Unavailable' ? 'selected' : '' }}" data-status="Unavailable">
                                <label class="card-body d-flex align-items-center" style="cursor: pointer;">
                                    <input type="radio" name="size_status" value="Unavailable" class="form-check-input me-3" {{ $sizeShoes->size_status === 'Unavailable' ? 'checked' : '' }}>
                                    <div>
                                        <h6 class="card-title mb-1">
                ...
                <button type="submit" class="btn btn-primary w-100">
                    <i class="bi bi-check-circle-fill me-2"></i>Update Shoe Size
                </button>
            </form>
        </div>
    </div>
