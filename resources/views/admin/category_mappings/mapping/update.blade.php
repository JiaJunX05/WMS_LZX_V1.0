                ...
                <div class="row align-items-center">
                    <div class="col-lg-8">
                        <div class="d-flex align-items-center">
                            ...
                            <div>
                                <h2 class="dashboard-title mb-1">Update Category Mapping</h2>
                                <p class="dashboard-subtitle mb-0">Modify the relationship between categories and subcategories</p>
                            </div>
                        </div>
                    </div>
        ...
        <div class="row g-0">
            ...
            <div class="col-md-7">
                <div class="card-body p-4">
                    ...
                    <!-- 表单内容 -->
                    <form action="{{ route('admin.category_mapping.mapping.update', $mapping->id) }}" method="post">
                        @csrf
                        @method('PUT')
                                ...
                                <select class="form-select border-start-0" id="category_id" name="category_id" required>
                                    ...
                                    @foreach($categories as $category)
                                        <option value="{{ $category->id }}"
                                                ...
                                                {{ $category->category_status === 'Unavailable' && $mapping->category_id != $category->id ? 'disabled' : '' }}
                                                data-status="{{ $category->category_status }}">
                                            {{ strtoupper($category->category_name) }}
                                            @if($category->category_status === 'Unavailable' && $mapping->category_id != $category->id)
                                                (Unavailable)
                                            @endif
                                        </option>
                                    @endforeach
                                ...
                                <select class="form-select border-start-0" id="subcategory_id" name="subcategory_id" required>
                                    ...
                                    @foreach($subcategories as $subcategory)
                                        <option value="{{ $subcategory->id }}"
                                                ...
                                                {{ $subcategory->subcategory_status === 'Unavailable' && $mapping->subcategory_id != $subcategory->id ? 'disabled' : '' }}
                                                data-status="{{ $subcategory->subcategory_status }}">
                                            {{ strtoupper($subcategory->subcategory_name) }}
                                            @if($subcategory->subcategory_status === 'Unavailable' && $mapping->subcategory_id != $subcategory->id)
                                                (Unavailable)
                                            @endif
                                        </option>
                                    @endforeach
