                        ...
                        <div class="card-body p-4">
                            ...
                            @if($showRoleSelection)
                                ...
                                @if($currentUserRole === 'SuperAdmin' && !$isUpdatingSelf)
                                    {{-- 超级管理员可以修改其他用户角色 --}}
                                    @include('components.form-templates.templates.role-selector', [
                                        'fieldName' => 'account_role',
                                        'currentRole' => $currentUserAccountRole,
                                        'label' => 'User Role',
                                        'helpText' => 'You can modify user roles to Staff, Admin, or Super Admin'
                                    ])
                                @else
                                    {{-- 非超级管理员或更新自己时，只显示当前角色 --}}
                                    @include('components.form-templates.templates.readonly-role-card', [
                                        'role' => $currentUserAccountRole,
                                        'label' => 'User Role',
                                        'helpText' => 'You can only modify user basic information, cannot change user roles'
                                    ])
                                @endif
                            @endif
                            ...
                            @if($showStatusSelection && !$isUpdatingSelf)
                                ...
                                @include('components.form-templates.templates.status-selector', [
                                    ...
                                    'helpText' => 'Choose whether the user can access the system'
                                ])
                            @elseif($showStatusSelection && $isUpdatingSelf)
                                ...
                                @php
                                    $currentStatus = $user->account->account_status ?? 'Available';
                                @endphp
                                @include('components.form-templates.templates.readonly-status-card', [
                                    'status' => $currentStatus,
                                    'label' => 'Account Status',
                                    'helpText' => 'You cannot change your own account status'
                                ])
                            @endif
